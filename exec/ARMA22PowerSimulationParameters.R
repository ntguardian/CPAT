#!/usr/bin/Rscript
################################################################################
# ARMA22PowerSimulationParameters.R
################################################################################
# 2018-09-24
# Curtis Miller
################################################################################
# Generate a .Rda file containing objects with data for parameters to be passed
# to PowerSimulations.R; this file containes ARMA(2,2) DGP settings.
################################################################################
  ...
  distfunc <- list(
    "arma22_0.1_0.5_0.3_0.2" = list(
        ...
        dist = function(n, mu) {return(as.vector(arima.sim(
                 list(order = c(2,0,2), ar = c(0.1, 0.5), ma = c(0.3, 0.2)),
                 n.start = 200, n = n)) + mu)},
        meanparam = "mu"
    )
  )
